...
namespace Enhanced.Rendering.RendererFeatures
{
    public class SSR : ScriptableRendererFeature
    {
        ...
        private SSRRenderPass renderPass = default;

        public override void Create()
        {
            renderPass = new SSRRenderPass(material);
        ...
        private class SSRRenderPass : ScriptableRenderPass
        {
            ...
            public SSRRenderPass(Material material)
            {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                firstPassTextureHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, commonRenderTextureDescriptor, "_SSR_FirstPassTexture", false);
                secondPassTextureHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, commonRenderTextureDescriptor, "_SSR_SecondPassTexture", false);
                ...
                using (var builder = renderGraph.AddRasterRenderPass<PassData>("SSRRaymarchPass", out var passData))
                {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<PassData>("SSRCompositePass", out var passData))
                {
                    passData.source = resourceData.cameraColor;
                    ...
                    builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                    {
                        material.SetTexture("_SSR_FirstPassTexture", firstPassTextureHandle);
                        material.SetTexture("_GBuffer2Texture", gBuffer2TextureHandle);
